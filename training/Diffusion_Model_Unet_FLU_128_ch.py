...
def fixed_sinusoidal_embedding(dim, max_pos=10000):
    """Return (max_pos, dim) sinusoidal table"""
    half = dim // 2
...
def apply_rotary_pos_emb(q, k, sinusoidal_pos):
    """
    q, k: tensors (..., L, head_dim)
    sinusoidal_pos: (L, head_dim) or broadcastable
    Applies RoPE to q and k and returns rotated versions.
    """
    # assume head_dim even
    # split last dim into pairs (x1,x2) and rotate
    # sinusoidal_pos expected shape (L, head_dim)
    # reshape to allow pairwise rotation
    L = q.size(-2)
    hd = q.size(-1)
...
class FiLM(nn.Module):
    """FiLM: generate scale and shift from condition vector"""
...
class LightweightRoPEAttention(nn.Module):
    def __init__(self, dim, num_heads=4, max_pos=8192):
        ...
        # precompute sinusoidal table for RoPE
...
class UpConv1D_4(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.up = nn.Sequential(
            nn.Upsample(size=141, mode='nearest'),
            nn.Conv1d(in_ch, out_ch, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
        )
...
class UpConv1D_3(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.up = nn.Sequential(
            nn.Upsample(size=291, mode='nearest'),
            nn.Conv1d(in_ch, out_ch, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
        )
...
class UpConv1D_2(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.up = nn.Sequential(
            nn.Upsample(size=565, mode='nearest'),
            nn.Conv1d(in_ch, out_ch, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
        )
...
class UNetScoreBasedDiffusionModel(nn.Module):
    def __init__(self, input_size=20, hidden_size=64, timesteps=1000, cond_dim: Optional[int] = None):
        ...
        n1 = 128  ## no. of feature channels for the input conlutional layer
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
        
        # encoder convs (with FiLM conditioning)
        ...
        self.Conv5 = ConvBlock1DFiLM(filters[3], filters[4], cond_dim=None)

        # lightweight RoPE attention in bottleneck to increase receptive field
        self.bottleneck_attn = LightweightRoPEAttention(filters[4], num_heads=8, max_pos=self.seq_len//16 + 4)

        # decoder
        self.Up5 = UpConv1D(filters[4], filters[3])
        self.Up_conv5 = ConvBlock1DFiLM(filters[4], filters[3], cond_dim=None)

        self.Up4 = UpConv1D_4(filters[3], filters[2])
